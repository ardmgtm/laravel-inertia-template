<template>
    <Dialog :header="(!editMode ? 'Add' : 'Edit') + ' User'" v-model:visible="dialogVisible" class="w-full max-w-xl" modal>
        <Form class="flex flex-col gap-2" :resolver @submit="(e) => !editMode ? addSubmitAction(e) : editSubmitAction(e)">
            <FormField name="name" v-slot="$field" class="flex">
                <label class="flex-none font-bold w-48" for="name">Name</label>
                <div class="flex-1">
                    <InputText id="name" placeholder="Name" v-model="formData.name" type="text" class="w-full" fluid @change="clearError('name')"/>
            ...
            <FormField name="username" v-slot="$field" class="flex">
                ...
                <div class="flex-1">
                    <InputText id="username" placeholder="Username" v-model="formData.username" type="text"
                        class="w-full" fluid @change="clearError('username')"/>
                    <Message class="h-2 mt-2" severity="error" size="small" variant="simple">
            ...
            <FormField name="password" v-slot="$field" class="flex">
                ...
                <div class="flex-1">
                    <Password id="password" placeholder="Password" v-model="formData.password" :feedback="false"
                        class="w-full" toggleMask fluid @change="clearError('password')"/>
                    <Message class="h-2 mt-2" severity="error" size="small" variant="simple">
